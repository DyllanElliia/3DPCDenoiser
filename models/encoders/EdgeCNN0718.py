...
class FeatureExtraction(Module):
  ...
  def __init__(self, k=32, input_dim=0, embedding_dim=512, global_conv=False):
    ...
    if self.global_conv:
      self.conv11 = DynamicEdgeConv(16, 16)
      self.conv22 = DynamicEdgeConv(48, 48)
      self.conv33 = DynamicEdgeConv(64, 64)
      self.conv44 = DynamicEdgeConv(128, 128)
  ...
  def conv_forward(self, x, conv_block):
    edge_index = self.get_edge_index(x)
    x = x.view(self.batch_size * self.num_points, -1)
    x_ = conv_block(x, edge_index)
    x_ = x_.view(self.batch_size, self.num_points, -1)
    return x_
  ...
  def forward(self, x):
    ...
    x1 = self.conv_forward(x, self.conv1)
    x2 = self.conv_forward(x1, self.conv2)
    x3 = self.conv_forward(x2, self.conv3)
    x4 = self.conv_forward(x3, self.conv4)

    if self.global_conv:
      x1 = self.conv_forward(x1, self.conv11)
      x2 = self.conv_forward(x2, self.conv22)
      x3 = self.conv_forward(x3, self.conv33)
      x4 = self.conv_forward(x4, self.conv44)
