...
from torch_geometric.nn.inits import reset, kaiming_uniform
# from ..utils import *
...
class DenseEdgeConv(Module):
  ...
  def __init__(
    ...
    if relative_feat_only:
      self.layer_first = FCLayer(2 * in_channels,
                                 growth_rate,
                                 bias=True,
  ...
  def get_edge_feature(self, x, knn_idx):
    ...
    if self.relative_feat_only:
      edge_feat = torch.cat([x_tiled, knn_feat - x_tiled], dim=3)
    else:
