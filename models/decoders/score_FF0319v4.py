...
class ScoreNet(nn.Module):
  ...
  def __init__(
    ...
    self.pos_e_dim = 64
    self.pos_embedding = nn.Linear(dim + 1, self.pos_e_dim)
    ...
    #   self.actvn_out = nn.ReLU()
    self.decode_net = nn.Sequential(nn.BatchNorm1d(hidden_size), nn.ReLU(),
                                    nn.Conv1d(hidden_size, 128, 1),
                                    nn.BatchNorm1d(128), nn.ReLU(),
                                    nn.Conv1d(128, 64, 1), nn.BatchNorm1d(64),
                                    nn.ReLU(), nn.Conv1d(64, 3, 1))
    print("fusion model", fusion_model)
    self.fusion_weight = nn.Sequential(nn.BatchNorm1d(hidden_size), nn.ReLU(),
                                       nn.Conv1d(hidden_size, 1, 1),
                                       fusion_model)

  def forward(self, x, c, flag=False):
    """
        ...
        """
    if flag:
      return self.forward_2(x, c)
    batch_size, num_points, _ = x.size()
    p = self.pos_embedding(
        torch.cat([x, torch.norm(x - x[:, :1, :], dim=-1, keepdim=True)],
                  dim=-1)).transpose(1, 2)  # (bs, dim, n_points)
    ...
    out = self.decode_net(net).transpose(1, 2)
    return out, w

  def forward_2(self, x, c):
    """
        :param x: (bs, npoints, self.dim) Input coordinate (xyz)
        :param c: (bs, npoints, self.zdim) Shape latent code
        :return: (bs, npoints, self.dim) Gradient (self.dim dimension)
        """
    # print("here 2")
    batch_size, num_points, _ = x.size()
