...
def normalize_sphere(pc, radius=1.0):
  ...
  pc = pc - center
  ## Scale
  scale = (pc**2).sum(dim=-1, keepdim=True).sqrt().max(
      dim=-2, keepdim=True)[0] / radius  # (B, N, 1)
  pc = pc / scale
...
def point_mesh_bidir_distance_single_unit_sphere(pcl, verts, faces):
  """
    ...
    """
  assert (pcl.dim() == 2 and verts.dim() == 2 and
          faces.dim() == 2), "Batch is not supported."
  ...
  return pytorch3d.loss.point_mesh_face_distance(meshes, pcls, 0)
...
def F_score_unit_sphere(gen, ref):
  """
    Args:
        gen:    (B, N, 3)
        ref:    (B, N, 3)
  ...
  dists_ab, _, _ = pytorch3d.ops.knn_points(ref, gen, K=1)  # (B, N, 1, 1)
  dists_ab = dists_ab.reshape(B, -1, 1)  # (B, N, 1)
  # print(dists_ab)

  dists_ba, _, _ = pytorch3d.ops.knn_points(gen, ref, K=1)
  dists_ba = dists_ba.reshape(B, -1, 1)  # (B, N, 1)
  # print(dists_ba)

  thresholds = np.linspace(1e-4, 1e-3, 10)

  p_a = get_threshold_percentage(dists_ab.cpu().numpy(), thresholds)
  p_b = get_threshold_percentage(dists_ba.cpu().numpy(), thresholds)
  F = [2 * p_a[i] * p_b[i] / (p_a[i] + p_b[i]) for i in range(len(p_a))]
