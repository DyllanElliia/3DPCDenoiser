...
class DiffusionSB2FF(Module):
  ...
  def __init__(self, args, var_sched: VarianceSchedule):
    ...
    if args.tag == "2FF?":
      print("FF0528QKL")
      from .decoders.score_FF0606 import ScoreNet
  ...
  def get_loss(self, x_0, T=None):
    ...
    feat = self.fusion(feat, feat, x_t_xt_p_index,
    ...
    gt_offset = x_t_xt_p_index.unsqueeze(2) - clean_Patchs  # (B, n, C, 3)
    gt_offset = gt_offset.mean(2)  # (B, n, 3)
    gt_target = -1 * gt_offset

    sig2 = ((self.alpha_mix +
             (1 - self.alpha_mix) * sa_b_t / sa_b_i_t)**2).view(-1, 1, 1)
    ...
    feat = self.fusion(feat, feat_t, x_t_xt_p_index, t_s)
    F_n = feat.size(-1)
  ...
  def get_score_end2end(
    ...
    with torch.no_grad():
      ...
      feat_T = self.fusion(feat_T, feat_T, x_T,
                           torch.tensor([1.0]).to(x_T.device))
      F_n = feat_T.size(-1)
      score = self.SB_GridEstimate(
          x_T,
          x_T,
          x_T,
          feat_T,
  ...
  def get_grad_end2end(
    ...
    with torch.no_grad():
      ...
      feat_T = self.fusion(feat_T, feat_T, x_T,
                           torch.tensor([1.0]).to(x_T.device))
      F_n = feat_T.size(-1)
      grad_pred = self.SB_GridEstimate(
          x_T,
          x_T,
          x_T,
          feat_T,
