...
class NormalizeUnitSphere(object):
    ...
    def normalize(pcl, center=None, scale=None):
        ...
        if scale is None:
            scale = (
                (pcl**2).sum(dim=1, keepdim=True).sqrt().max(dim=0, keepdim=True)[0]
            )  # (1, 1)
        pcl = pcl / scale
    ...
    def __call__(self, data):
        assert (
            'pcl_noisy' not in data
        ), 'Point clouds must be normalized before applying noise perturbation.'
...
class NormalizeDiagonalLength(object):
    def __init__(self):
        super().__init__()
    ...
    def normalize(pcl, center=None, scale=None):
        ...
        pcl = pcl - center
        if scale is None:
            scale = (p_max - p_min).norm(dim=-1, keepdim=True)  # (1, 1)
        pcl = pcl / scale
        return pcl, center, scale
...
class AddNoise(object):
    ...
    def __call__(self, data):
        ...
        data['pcl_noisy'] = (
            data['pcl_clean'] + torch.randn_like(data['pcl_clean']) * noise_std
        )
...
class AddLaplacianNoise(object):
    ...
    def __call__(self, data):
        ...
        noise = torch.FloatTensor(
            np.random.laplace(0, noise_std, size=data['pcl_clean'].shape)
        ).to(data['pcl_clean'])
...
class AddCovNoise(object):
    ...
    def __call__(self, data):
        ...
        noise = np.random.multivariate_normal(
            np.zeros(3), self.cov.numpy(), num_points
        )  # (N, 3)
        noise = torch.FloatTensor(noise).to(data['pcl_clean'])
